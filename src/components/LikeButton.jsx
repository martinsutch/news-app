...
const LikeButton = ({likes , setLikes, article_id}) => {
    ...
    const handleLike = () => {
        ...
        if (liked) {
            ...
            .catch(() => {
                setLiked(true)
                setLikes(likes)
                setStatus({icon: "error", msg: "There was an issue removing your like, please try again.", colour: "red", duration: 0})
                setDisabled(false)
            })
        } else {
            setLiked(true)
            ...
            .catch(() => {
                setLiked(false)
                setLikes(likes)
                setStatus({icon: "error", msg: "There was an issue liking this article, please try again.", colour: "red", duration: 0})
                setDisabled(false)
            })
        }
    }
    ...
    return (
        <div onClick={handleLike} className={`button internal-row ${disabled ? "disabled" : ""}`}>
